% sample_latency_monitor.m
%
% Example MATLAB script to monitor latency metrics for satellite payloads
% Returns JSON with latency measurements for the specified payload

function sample_latency_monitor(scid)
    ...
    % Threshold for latency metric (in a real system this would be read from a config)
    ...
    for i = 1:length(payloads)
        ...
        % Simulate a latency reading (normally around 200ms but sometimes higher)
        base_latency = 200;
        variance = rand() * 100;  % Random value between 0 and 100
        latency_value = base_latency + variance;
